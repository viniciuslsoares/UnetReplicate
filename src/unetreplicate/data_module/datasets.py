...
class BasicDataset(Dataset):
    def __init__(
        ...
        if external_ids is not None:
            ...
            if not self.ids:
                raise RuntimeError(
                    f"No input file found in {data_dir}, make sure you put your images there"
                )
        ...
        with Pool() as p:
            unique = list(
                tqdm(
                    p.imap(
                        partial(unique_mask_values, mask_dir=self.mask_dir), self.ids
                    ),
                    total=len(self.ids),
                )
            )
    ...
    def __getitem__(self, idx):
        ...
        assert (
            len(data_file) == 1
        ), f"Either no image or multiple images found for the ID {name}: {data_file}"
        ...
        return {"image": data, "mask": mask.squeeze()}
