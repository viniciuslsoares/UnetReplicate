...
def evaluate_model(
    ...
    for batch in tqdm(
        ...
        # print(image.shape)
        if window:
            image, mask_true = batch["image"][:, 0], batch["mask"].squeeze(
                dim=0
            )  # Remove the gradient channel
            ...
            image = TF.pad(
                image, padding=(pad_size, 0, pad_size, 0), padding_mode="symmetric"
            ).to(
                device
            )  # Faixa de reflexão
            ...
            for i, crop in enumerate(crops):
                grad = (
                    torch.tensor(generate_gradient(crop.shape[1:]))[:, :, 1]
                    .unsqueeze(dim=0)
                    .to(device=device)
                )
    ...
    net.train()
    return (
        float(IoU_sum / max(num_val_batches, 1)),
        float(F1_sum / max(num_val_batches, 1)),
        loss.item(),
    )
