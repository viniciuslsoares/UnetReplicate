...
def train_model(
    ...
    ids = set(
        [
            splitext(file)[0]
            for file in listdir(dir_img_train)
            if isfile(join(dir_img_train, file)) and not file.startswith(".")
        ]
    )
    ...
    for split in range(k_folds):
        ...
        if k_folds == 1:
            dataset = BasicDataset(
                data_dir=dir_img_train,
                mask_dir=dir_mask_train,
            ...
            train_set = BasicDataset(
                ...
                mask_dir=dir_mask_train,
                augmentation=augmentation,
                external_ids=train_ids,
            )
            val_set = BasicDataset(
                data_dir=dir_img_train,
                mask_dir=dir_mask_train,
                augmentation=False,
                external_ids=val_ids,
            )
        ...
        loader_args = dict(
            batch_size=1, num_workers=os.cpu_count(), pin_memory=True
        )  # Batch size = 1 por causa da janela
        ...
        minerva_dice = DiceLoss(
            mode="multiclass", classes=None, log_loss=False, from_logits=True
        )
        global_step = 0
        ...
        for epoch in range(1, epochs + 1):
            ...
            iou_sum = 0
            with tqdm(
                total=n_train, desc=f"Epoch {epoch}/{epochs}", unit="img"
            ) as pbar:
                for batch in train_loader:
                    ...
                    tv_loss = (
                        total_variation(F.softmax(masks_pred, dim=1).float())
                        / batch_size
                    )

                    minerva_dice_loss = minerva_dice(masks_pred, true_masks)
                    loss = (
                        minerva_dice_loss * DICE_WHEIGHT
                        + ce_loss * CROSS_ENTROPY_WHEIGHT
                    )
                    ...
                    if division_step > 0:
                        if global_step % division_step == 0:
                            ...
                            val_score = evaluate_model(
                                model, val_loader, device, window=False
                            )
                            scheduler.step(val_score[1])
                            logging.info(
                                "IoU score: {:.3f}; F1 score: {:.3f}".format(
                                    val_score[1], val_score[0]
                                )
                            )
                            ...
                            model.train()
                logging.info(
                    "Epoch finished ! IOU: {:.3f}".format(iou_sum / len(batch["image"]))
                )
            if save_checkpoint:
                ...
                torch.save(
                    state_dict, str(dir_checkpoint + f"/checkpoint_{name}{epoch}.pth")
                )
        ...
        plot_and_save(
            ...
            x2=eval_loss_list,
            x2_name="Eval Loss",
            title="Train x Eval Loss",
            filename=f"outputs/train_eval_loss_{name}.png",
        )
