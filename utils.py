...
def get_input_data(filename) -> np.ndarray:
    ...
    return np.concatenate(
        [np.expand_dims(data, axis=2), np.expand_dims(gradient, axis=2)], axis=2
    )
...
def reconstruct_image(
    list_coords, list_images, n_classes, out_height, out_width, device
):
    ...
    for i, (x, y) in enumerate(list_coords):
        temp = (
            Func.softmax(torch.tensor(list_images[i]), dim=0).squeeze(dim=0).to(device)
        )  # Pvvment aplicar unsqueeze(dim=0)
...
def plot_and_save(
    x1, x1_name, filename, x2=None, x2_name=None, x3=None, x3_name=None, title="title"
):
